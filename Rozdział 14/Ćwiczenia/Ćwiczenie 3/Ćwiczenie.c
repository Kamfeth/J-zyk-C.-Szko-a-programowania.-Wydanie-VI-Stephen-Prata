...
int main(void)
{
     ...
     if (licznik > 0)
     {
          qsort(bibl, licznik, sizeof(struct ksiazka), (__compar_fn_t)strcmp);
          float calkowitaWartoscKsiazek = 0.0;
          ...
          for (index = 0; index < licznik; index++)
          {
            ...
            calkowitaWartoscKsiazek += bibl[index].wartosc;
          }
          printf("\nCałkowita wartość wszystkich wprowadzonych książek wynosi: %.2f zl", calkowitaWartoscKsiazek);
     }
     else
